...
var storage = multer.diskStorage({
    destination: (req, file, cb) => {
      cb(null, '../contractor/public/images/uploads')
    },
...
var upload = multer({storage: storage}).fields([{ name: 'image', maxCount: 1 }, { name: 'photos', maxCount: 8 }])
    ...
    let err, company;
       ...
       company_info.user = user._id;
        upload(req,res,function(err) {
            ...
            if(req.files){
                company_info.logo =  req.files.image[0].filename;
                var pictures =[];
                var photos = req.files.photos;
                for(var val of photos){
                     pictures.push(val.filename);
                }
